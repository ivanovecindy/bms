...
public class DateUtil {
	...
	/** 时间格式年月日时分秒毫秒 **/
	private static SimpleDateFormat SDF_NYRSFMH = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss.SSS");
	/** 时间格式只有年月日 **/
	private static SimpleDateFormat SDF_NYR = new SimpleDateFormat("yyyy-MM-dd");
	/** 时间格式只有年月日中文格式 **/
	private static SimpleDateFormat SDF_NYRZW = new SimpleDateFormat("yyyy年MM月dd日");
	/** 时间格式年月日时分秒 **/
	private static SimpleDateFormat SDF_NYRSFM = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	/** 时间格式只有年月日 **/
	private static SimpleDateFormat SDF_N = new SimpleDateFormat("yyyy");
	...
	public static String getFirstDayOfYear(Date date,String dateForma){
		Calendar calender = Calendar.getInstance();
		calender.setTime(date);
		calender.set(Calendar.DAY_OF_YEAR, calender.getActualMinimum(Calendar.DAY_OF_YEAR));
		setStartTimeOfDay(calender);
		 SimpleDateFormat f = new SimpleDateFormat(dateForma);
		 return f.format(calender.getTime()); 
	}
	...
	public static String getNowDate(String dateFormat,int day) {
		Date date = new Date();
	...
	public static String getNowMonth(String dateFormat,int month) {
		Date date = new Date();
        Calendar c = Calendar.getInstance();  
        c.add(Calendar.MONTH, month); //日期加1天  
        date = c.getTime();  
		SimpleDateFormat f = new SimpleDateFormat(dateFormat);
		return f.format(date);
	 ...
	 * @return
	 */
	public static List<String> getYearsWeek(int year) {
		...
		while (year >= c.get(Calendar.YEAR)) {
			if (c.get(Calendar.DAY_OF_WEEK) == 1
				...
				list.add(c.get(Calendar.YEAR)+"-"+month+"-"+date);
				/*System.out.println(c.get(Calendar.YEAR) + "-"
						+ month + "-"
						+ date
						+ (c.get(Calendar.DAY_OF_WEEK) == 1 ? "是星期天" : "是星期六"));*/
			}
	 ...
	 * @return
	 */
	public static boolean compareDate(Date date,int n) {
		Calendar c = Calendar.getInstance();
		c.setTime(date);
		c.add(Calendar.MONTH,n);
		Date date2=c.getTime();//date减一个月
		if (new Date().getTime()>date2.getTime()&&new Date().getTime()<date.getTime()) {
			return true;
		}
		return false;
	}
...
//		Date d2 = parseToDate("2007-12-29", null);
	/*	System.out.println(formatDate(getFirstDayOfMonth(2010,10),"yyyy-MM-dd HH:mm:ss.SSS"));
		System.out.println(formatDate(getLastDayOfMonth(2010,10),"yyyy-MM-dd HH:mm:ss.SSS"));
		...
		System.out.println(formatDate(getEndTimeOfDay(new Date()),"yyyy-MM-dd"));
		Date date = new Date();
        Calendar c = Calendar.getInstance(); 
        c.setTime(date);
		List <String>list  = DateUtil.getYearsWeek(2016);
		for (String str : list) {
			Date d1 = parseToDate(str, null);
			System.out.println(formatDate(d1,"yyyyMMdd")+"--"+formatDate(d1,"yyyy")+"---"+formatDate(d1,"MM"));
			 
			 
		}*/ 
		System.out.println("2016012345".substring(0, 8));
	}
}
