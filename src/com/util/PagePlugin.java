			...
			if(mappedStatement.getId().matches(pageSqlId)){ //拦截需要分页的SQL
				...
				if(parameterObject==null){
					...
					BoundSql countBS = new BoundSql(mappedStatement.getConfiguration(),countSql,boundSql.getParameterMappings(),parameterObject);
				      Field metaParamsField = com.util.ReflectUtil.getFieldByFieldName(boundSql, "metaParameters");
			            if (metaParamsField != null) {
			                MetaObject mo = (MetaObject) com.util.ReflectUtil.getValueByFieldName(boundSql, "metaParameters");
			                com.util.ReflectUtil.setValueByFieldName(countBS, "metaParameters", mo);
			            }
					...
					if(parameterObject instanceof Page){	//参数就是Page实体
						...
						Field pageField = ReflectHelper.getDeclaredField(parameterObject,"page");
						if(pageField!=null){
							page = (Page) ReflectHelper.getFieldValue(parameterObject,"page");
							if(page==null)
								page = new Page();
							page.setEntityOrField(false); //见com.flf.entity.Page.entityOrField 注释
							page.setTotalResult(count);
							ReflectHelper.setFieldValue(parameterObject,"page", page); //通过反射，对实体对象设置分页对象
						}else{
							throw new NoSuchFieldException(parameterObject.getClass().getName()+"不存在 page 属性！");
						}
					}
					String pageSql = generatePageSql(sql,page);
					ReflectHelper.setFieldValue(boundSql, "sql", pageSql); //将分页sql语句反射回BoundSql.
				}
			}
		}
		return ivk.proceed();
