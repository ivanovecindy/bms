...
<div style="margin-top: 3px;">	<jsp:include page="../../jsp/system/title.jsp"></jsp:include> </div><br>
            <form action="${ctx}/modelDeal/modelList.action" method="post" name="roleForm" id="roleForm">
                ...
                <div id="button"  style="text-align: left;margin-left: 5px;margin-bottom: 5px;">
                    <a href="javascript:void(0);" onclick="addmenu('','add')"  target="rightFrame"  class="ext_btn"><span class="add"></span>添加</a>
                  </div>
                ...
                <table class="list_table" >
                        ...
                        <tbody>
                        <c:choose>
                            <c:when test="${not empty childModelDealList}">
                                <c:forEach items="${childModelDealList}" var="modelDeas" varStatus="vs">
                                    <tr >
                                        ...
                                        <td nowrap="nowrap"><a href="javascript:void(0);" onclick="addmenu('${modelDeas.id}','update')" target="rightFrame" target="rightFrame" class="tablelink">修改</a> <a href="javascript:void(0);" onclick="modelDealView('${modelDeas.id}')" target="rightFrame" target="rightFrame"  class="tablelink">查看</a> <a href="javascript:void(0)" onclick="delMenu('${modelDeas.id}');"  class="tablelink"> 删除</a>
                                    </tr>
                                </c:forEach>
                            </c:when>
                            <c:otherwise>
